...
export const MEMORY_INFLUENCE_GATE_VERSION = "memory_influence_gate_v1";
export const DEFAULT_MEMORY_INFLUENCE_CASE_COUNT = 4;
export const DEFAULT_MEMORY_HIT_RATE_MIN = 0.8;
export const DEFAULT_ACTION_CHANGED_RATE_MIN = 0.5;
...
export async function buildMemoryInfluenceReport({
  caseCount = DEFAULT_MEMORY_INFLUENCE_CASE_COUNT,
  ...
  try {
    ...
    const [
      ...
      import("./planner-user-input-edge.mjs"),
      import("./executive-memory.mjs"),
    ]);
    ...
    const memoryHitRate = safeRatio(memoryHitCount, memoryEligibleCount);
    const actionChangedByMemoryRate = safeRatio(actionChangedCount, memoryHitCount);
...
export function renderMemoryInfluenceReport(report = {}) {
  const lines = [
