...
function collectKeywordLineWindow(block = "", keyword = "") {
  ...
  while (end + 1 < lines.length) {
    ...
    if (/^[A-Za-z][A-Za-z\s/_-]*[:：]\s*$/.test(nextLine)) {
      break;
    }
    end += 1;
    if (/[.。!?！？]$/.test(lines[end].trim())) {
      break;
    }
  }
...
function isLowValueSnippet(text) {
  ...
  if (/^(module|path|file|api|runtime)\s*[:/-]\s*$/i.test(t)) {
    return true;
  }
  ...
  if (!/[.。!?！？:：]/.test(t) && /\b(runbook|guide|readme)\b/i.test(t)) {
    return true;
  }
  if (!/[.。!?！？:：]/.test(t) && (t.match(/[A-Za-z0-9]+/g) || []).length <= 3 && !/[\u4e00-\u9fff]/.test(t)) {
    return true;
  }
  return false;
}
...
export function querySystemKnowledge(keyword) {
  const index = getSystemKnowledgeIndex();
  const hits = searchDocsByKeyword(index, keyword);
  return rankResults(hits, keyword);
}

export function querySystemKnowledgeWithSnippet(keyword) {
  return querySystemKnowledge(keyword).slice(0, 3).map((doc) => ({
    id: doc.id,
...
export function filterKnowledgeContextResults(results = [], keyword = "") {
  return (Array.isArray(results) ? results : [])
    .filter((result) => !keyword || isRelevantSnippet(result?.snippet, keyword))
...
export function querySystemKnowledgeWithContext(keyword) {
  ...
  const buckets = queries
    .map((query) => ({
      ...
      results: filterKnowledgeContextResults(
        querySystemKnowledgeWithSnippet(query),
        query,
      ),
    }))
