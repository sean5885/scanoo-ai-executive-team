...
test("runtime-info naming stays canonical across planner envelope and answer boundary", () => {
  ...
  assert.deepEqual(Object.keys(userResponse).sort(), ["answer", "limitations", "ok", "sources"]);
  assert.doesNotMatch(JSON.stringify(userResponse), /"action":"get_runtime_info"|"action":"runtime_info"|"kind":"get_runtime_info"|"kind":"runtime_info"/);
