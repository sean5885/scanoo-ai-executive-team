...
test("router prefers active-doc follow-up over search when query points to current file", () => {
  assert.deepEqual(
    route("查一下這份文件內容", { activeDoc: { doc_id: "doc_123", title: "Demo" } }),
    {
      action: "get_company_brain_doc_detail",
...
test("planner flow runtime keeps themed search-plus-content query on generic search action", () => {
  const resolved = resolvePlannerFlowRoute({
    flows: [plannerRuntimeInfoFlow, plannerOkrFlow, plannerBdFlow, plannerDeliveryFlow, plannerDocQueryFlow],
    userIntent: "幫我找 OKR 進度內容",
    payload: {},
    logger: console,
  });
...
test("runPlannerToolFlow formats mixed search-and-detail result with title match reason and summary", async () => {
  ...
  const result = await runPlannerToolFlow({
    ...
    async presetRunner({ preset, input }) {
      assert.equal(preset, "search_and_detail_doc");
      assert.equal(input.q, "整理 onboarding 流程並解釋");
      return {
        ok: true,
        preset,
        steps: [
  ...
  assert.equal(result.execution_result?.formatted_output?.match_reason, "整理 onboarding 流程並解釋");
  assert.equal(
    result.execution_result?.formatted_output?.content_summary,
...
test("selectPlannerTool aligns search-plus-content phrasing to generic search", async () => {
  const { selectPlannerTool } = await import("../src/executive-planner.mjs");
  const result = selectPlannerTool({
    userIntent: "搜尋 onboarding 文件並打開內容",
    taskType: "",
    logger: console,
  });
