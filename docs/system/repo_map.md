...
- Mutation governance
  ...
  - `mutation-runtime.mjs` is currently only a narrow passthrough wrapper used by the `create_doc` HTTP execute path; it returns `missing_execute` when no executor is provided and otherwise wraps the downstream executor result with passthrough metadata. The `create_doc` route still unwraps the nested write result before sending the HTTP response, so it does not yet replace route-local guards, verifiers, admission, or execute logic.
