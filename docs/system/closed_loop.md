...
- downstream supporting-agent work items now run as bounded in-process parallel support steps inside `src/executive-orchestrator.mjs`, while merge-agent synthesis remains serial; this is still not a background worker mesh
...
- the verifier now reads only the task `execution_journal`; Phase-1 minimum journal fields are `classified_intent`, `selected_action`, `dispatched_actions`, `raw_evidence`, `fallback_used`, and `verifier_verdict`; orchestration observability also lands `parallel_step_count`, `total_step_count`, and `parallel_ratio`
