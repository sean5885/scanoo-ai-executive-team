...
- the same session store now also carries a minimal working-memory v1 block (`current_goal`, `inferred_task_type`, `last_selected_agent`, `last_selected_skill`, `last_tool_result_summary`, `unresolved_slots`, `next_best_action`, `confidence`, `updated_at`): pre-routing reads are fail-closed on missing/malformed snapshots, same-task retry/follow-up can reuse bounded prior routing hints, unresolved slots can steer next action selection, and write-back is centralized at the answer boundary as patch updates only (no mid-planner full overwrite)
