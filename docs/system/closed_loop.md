...
- the same session store now also carries a minimal working-memory v2 block (`task_id`, `task_type`, `task_phase`, `task_status`, `current_owner_agent`, `previous_owner_agent`, `handoff_reason`, `retry_count`, `retry_policy`, `slot_state`, plus v1-compatible fields): pre-routing now prioritizes running-task owner continuity, waiting-user slot-fill continuation, and bounded retry/reroute policy, while topic-switch phrasing marks the previous task as abandoned and starts a new task id; `slot_state.ttl` is enforced so stale slots do not affect later turns; write-back remains centralized at the answer boundary as patch updates only (no mid-planner full overwrite)
