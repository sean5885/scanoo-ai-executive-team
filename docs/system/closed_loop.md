...
- the same session store now also carries a minimal working-memory v2 block (`task_id`, `task_type`, `task_phase`, `task_status`, `current_owner_agent`, `previous_owner_agent`, `handoff_reason`, `retry_count`, `retry_policy`, `slot_state`, `execution_plan`, plus v1-compatible fields): `execution_plan` is session-level only and currently includes `plan_id`, `plan_status`, `current_step_id`, and ordered `steps[]` (`step_id`, `step_type`, `owner_agent`, `intended_action`, `status`, `depends_on`, `retryable`, `artifact_refs`, `slot_requirements`); pre-routing now prioritizes active-plan step continuation, waiting-user slot-fill resume, and bounded retry same-step resume before selector fallback, while topic-switch phrasing invalidates old plan context and starts a new task/plan id; `slot_state.ttl` is enforced so stale slots do not affect later turns; write-back remains centralized at the answer boundary as patch updates only (no mid-planner full overwrite, no standalone workflow engine)
