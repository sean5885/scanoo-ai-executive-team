...
- `src/executive-planner.mjs` now also exposes a minimal `runPlannerToolFlow(...)` helper that performs `select -> preset|dispatch / fallback`, returning `{ selected_action, execution_result, trace_id }` and logging under `stage=planner_end_to_end`
