...
- Current truth:
  ...
  - for direct-message `personal-assistant` turns whose lane plan would otherwise be `general_assistant_action`, `/Users/seanhan/Documents/Playground/src/lane-executor.mjs` now keeps the turn on `executePersonalAssistant(...)` and prefers one direct text-model reply through `/Users/seanhan/Documents/Playground/src/llm/generate-text.mjs` when the primary credential is present, instead of re-entering the planner answer edge; this smoke-driven change prevents generic strategy asks from being misread as company-brain/document lookups
  ...
  - the previously added planner-first DM health gate remains local-only in `/Users/seanhan/Documents/Playground/src/lane-executor.mjs`, but generic DM no longer depends on it for normal answer quality
