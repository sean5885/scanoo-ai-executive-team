...
- Responsibility:
  ...
  - the same create route now also treats post-create initial-content writes as fail-soft: if the doc itself was created but the follow-up `updateDocument(..., "replace")` write is rejected, the API still returns success for the created doc, exposes `initial_content_write_failed` plus structured error details, and only persists `raw_text` into the local lifecycle/index row when that content write actually succeeded
  ...
  - `src/read-runtime.mjs` now owns the checked-in unified read-runtime entry for single-authority reads: verified company-brain list/search/detail fix `primary_authority = "mirror"` with `authorities_attempted = ["mirror"]` and delegate only to `src/company-brain-query.mjs`; approved company-brain reads plus the current internal learning-state reads fix `primary_authority = "derived"` with `authorities_attempted = ["derived"]` and delegate to `src/derived-read-authority.mjs`; live doc/comment actions fix `primary_authority = "live"` with `authorities_attempted = ["live"]` and delegate only to `src/lark-content.mjs`; live reads require `freshness = "live_required"`, and the runtime does not mix mirror, derived, or live fallback in the same read
  - the same module now also exposes bounded synchronous wrappers for internal mirror/derived/index reads so existing mutation-side review/learning/verifier helpers and the checked-in `docs/system` knowledge helpers can re-enter the same runtime without changing their public synchronous contracts
  - `GET /api/doc/read`, `GET /api/doc/comments`, doc-rewrite preview/apply verification reads, comment-suggestion source reads, meeting doc reads, and lane-level direct doc reads now all enter that same `read-runtime.mjs` live branch instead of calling `lark-content.mjs` directly from the route/workflow layer
  - `GET /api/company-brain/docs` now exposes a minimal read-only list view over `company_brain_docs`, with `limit` support and `stage=company_brain_list` logging, but the route now reaches mirror data through `read-runtime.mjs` instead of direct repository helpers
  - `GET /api/company-brain/docs/:doc_id` now exposes a minimal read-only detail view over `company_brain_docs`, returning the same `{ doc_id, title, source, created_at, creator }` shape and logging `stage=company_brain_detail`, while routing through `read-runtime.mjs`
  - `GET /api/company-brain/search?q=...` now exposes a minimal read-only search view over `company_brain_docs`, matching against `title` and `doc_id`, reusing the same item schema, and logging `stage=company_brain_search`, while routing through `read-runtime.mjs`
  - `src/company-brain-query.mjs` now centralizes the mirror-side planner-facing company-brain list/search/detail actions, joining `company_brain_docs` with mirrored `lark_documents.raw_text` plus optional `company_brain_learning_state`, ranking search results with a composite score over keyword match, semantic-lite similarity, learned `key_concepts` / `tags`, and mirror recency, supporting `top_k` with default `5` (`limit` remains a compatibility alias), and emitting unified `{ success, data, error }` payloads that keep only structured summaries plus `learning_state`
  ...
  - `/agent/docs/create`, `/agent/company-brain/docs`, `/agent/company-brain/search`, `/agent/company-brain/docs/:doc_id`, `/agent/company-brain/approved/docs`, `/agent/company-brain/approved/search`, `/agent/company-brain/approved/docs/:doc_id`, `/agent/company-brain/learning/ingest`, `/agent/company-brain/learning/state`, and `/agent/system/runtime-info` now provide thin agent-facing bridges over the corresponding document/runtime/query routes, normalizing output into `{ ok, action, data, trace_id }`; all company-brain read bridges now route through `read-runtime.mjs`, while the learning routes additionally log `stage=company_brain_learning` and pass their final write hop through `mutation-runtime`
  ...
  - the same planner contract/runtime line now also keeps `create_doc` governance aligned as checked-in metadata (`external_write=true`, `confirm_required=true`, `review_required=conditional`, `required_entry_fields=["source","owner","intent","type"]`) across `planner_contract.json`, planner tool registry, and route contracts; the agent bridge blocks missing entry governance, while planner dispatch auto-fills the controlled planner path so the existing runtime create path does not regress
  ...
  - `src/planner-conversation-memory.mjs` now provides a minimal planner conversation summary layer backed by a small JSON file store keyed by `sessionKey`: it keeps per-session `latest_summary`, bounded `recent_messages`, and `last_compacted_at`, auto-loads persisted memory when planner runtime starts, writes back after compact/record updates, and exposes manual/auto compact helpers so planner prompt assembly can prefer same-session `latest_summary + recent_messages + current query` instead of replaying long history; the compacted summary now also preserves `active_theme`
  ...
  - `src/planner-doc-query-flow.mjs` now holds the reusable planner-side document query pipeline for company-brain reads: hard pre-route integration, session-scoped `active_doc` / `active_candidates` / `active_theme` context, doc-query payload shaping, ambiguity-aware result formatting, doc-query context sync after successful search/detail flows, and minimal internal debug tracing for route/result observability; its checked-in doc routing precedence is now `follow_up -> doc -> search -> fallback`, requires a unique resolved target, and keeps `搜尋/查詢 + 內容/打開` phrasing on generic `search_company_brain_docs` unless a same-session active doc/candidate follow-up already resolves a single detail action; detail-like formatted outputs can now also carry `learning_status`, `learning_concepts`, and `learning_tags` when available, but same-read formatting no longer supplements mirror results with `/api/doc/read`; `executive-planner.mjs` stays the public planner entrypoint and only wires this flow in, and now lazily restores same-session doc-query context from `latest_summary` when runtime context is empty
  ...
  - runtime-info naming is now fixed by surface: raw HTTP route/stage stays `runtime_info`, planner action and agent-bridge action stay `get_runtime_info`, and planner-formatted `kind` / flow id stay `runtime_info`
  - `src/planner-okr-flow.mjs` now holds the reusable planner-side OKR flow: it detects OKR/topic-style knowledge queries, shapes them into the existing company-brain document query actions, preserves doc follow-up detail precedence, and now keeps search-bearing summary/content phrasing on `search_company_brain_docs` instead of promoting it to `search_and_detail_doc`; it reuses the doc-query pipeline's context, ambiguity handling, and formatter instead of duplicating that logic in `executive-planner.mjs`
  - `src/planner-bd-flow.mjs` now holds the reusable planner-side BD flow: it detects BD / 商機 / 客戶 / 跟進 / demo / 提案 style knowledge queries, routes `整理|進度|跟進|分析` requests into `search_and_detail_doc` only when no search intent is present, otherwise uses `search_company_brain_docs`, and reuses the same doc-query pipeline/context instead of duplicating BD-specific read logic in `executive-planner.mjs`
  - `src/planner-delivery-flow.mjs` now holds the reusable planner-side delivery flow: it detects delivery/onboarding/SOP knowledge queries, preserves doc follow-up detail precedence, keeps search-bearing content/流程 phrasing on generic `search_company_brain_docs`, and reuses the doc-query pipeline's context, ambiguity handling, and formatter instead of duplicating that logic in `executive-planner.mjs`
  ...
  - `src/planner-task-lifecycle-v1.mjs` now mirrors `formatted_output.action_layer.next_actions` into a minimal planner-side lifecycle store backed by `.data/planner-task-lifecycle-v1.json`: each derived task keeps its derivation lifecycle (`created -> clarified -> planned`), a separate operational task state (`planned -> in_progress -> blocked -> done`), source metadata (`theme`, `selected_action`, `doc_id`, `trace_id`, extracted owner/deadline/risks/status`, `source_title`, `source_summary`), and a scope snapshot of the latest active task ids plus `last_active_task_id`; the same store now also carries a small `execution v1` layer for ongoing progress tracking (`progress_status`, `progress_summary`, `note`, `result`, `execution_started_at`, `last_progress_at`, `completed_at`, bounded `execution_history`) without adding DB or scheduler dependencies, plus a bounded pending-item reminder layer where unfinished items render as `{ item_id, label, status, actions }` and currently expose only `mark_resolved`; `handlePlannerPendingItemAction(...)` only supports that one action and flips local pending-item state from `pending` to `resolved` without changing planner routing, task assignment, or public planner result shape; the same module now also exposes a narrow external-pending sync path so non-planner flows such as cloud-doc can project their pending items into an isolated scope and reuse the exact same `mark_resolved` handler without changing planner architecture; follow-up task-oriented queries (`進度 / 誰負責 / 何時到期 / 這個卡住了 / 這個完成了 / 完成一半 / 已處理 / 結果 / 備註`) now read or update that local store before any doc follow-up dispatch, can single-target one task by `第一個 / 第二個 / 第N個`, `這個`, or unique `owner`, and when the target resolves to exactly one task they return single-task read summaries for `owner / deadline / status / result / note`; planner decision-side reads now resolve task context by `active_doc`, mentioned `source_title`, mentioned task title, `active_theme`, then latest scope, and task driving prefers that focused task for next-step suggestions; ambiguous target attempts return candidate task rows without mutating state, while the public planner output remains unchanged
...
- Responsibility:
  ...
  - the same planner runtime now also keeps a minimal session-scoped read context: within the same `sessionKey`, a successful `search_and_detail_doc` or `get_company_brain_doc_detail` can remember `active_doc = { doc_id, title }`; an ambiguous successful search can remember a bounded `active_candidates` list; themed knowledge flows can remember `active_theme = okr|bd|delivery`; follow-up pronoun/detail questions like `這份文件裡面寫了什麼` route directly to `get_company_brain_doc_detail`, ordinal follow-ups resolve only against the same-session candidates, and compacted planner memory can restore that same-session doc-query context after runtime reset/restart
  ...
  - the same module now also includes a minimal multi-step helper that accepts ordered planner-tool `steps`, dispatches them sequentially through the existing planner tool bridge, accepts either `step.params` or internal `step.payload`, returns `{ ok, steps, results, trace_id, error, stopped, stopped_at_step }`, stops on the first normalized step failure by default, and logs `stage=planner_multi_step`; strict planner hardening now collapses over-eager multi-step outputs back to a single action when the user request is clearly `runtime-only`, `list-only`, `search-only`, or plain `create_doc`
...
- Responsibility:
  ...
  - cloud-doc review now treats obvious test-residual titles containing `Demo`, `Verify`, `Retry`, or `verify_failed` as skipped artifacts, excludes them from `待人工確認`, and surfaces only a compact skipped-count note in the reply summary
  ...
  - cloud-doc follow-up tracing now emits one route-level runtime log with the resolved workflow/action/session state and one reply-level runtime log with `workflow_hit`, `reply_builder_name`, `final_text_source_function`, plus cache-hit state, so operators can distinguish routing misses from cached/old reply text
...
- Responsibility:
  ...
  - keep `mutation-runtime.mjs` as the shared write-entry scaffold for the first connected family; it now normalizes the checked-in canonical request, runs admission before execute, returns `missing_execute` when no executor is supplied, returns `invalid_executor` with a stable message when `execute` is present but not callable, derives `meta.execution_mode` from `context.execution_mode` with a default of `passthrough`, records `meta.duration_ms` around the downstream executor call, and emits a small `meta.journal` (`action / status / started_at`, plus `error` on failure); execution-failure fail-soft results now support an optional `context.rollback` hook and record `journal.rollback.status` as `success`, `failed`, or `pending` when no rollback hook is supplied; when callers pass `context.idempotency_key`, the same runtime now keeps an in-process state entry (`pending` before execute, `done` after the first successful response) so overlapping retries fail-soft with `idempotency_in_progress` instead of double-running the mutation, and non-success paths clear the pending entry so later retries can re-enter safely
...
- `/Users/seanhan/Documents/Playground/src/knowledge/snippet-cleaner.mjs`
  ...
  - unwraps Markdown links/images while keeping visible text, strips navigation lines, list shells, stray backticks, path-only lines, checkpoint-like metadata lines, and module/title prefixes such as `runtime / monitoring - Purpose:`
  - drops dangling or incomplete lines, merges the remaining meaningful lines, prefers the sentence segment containing the active keyword when available, trims broken edges, and caps the final preview near a sentence/line boundary at roughly 220 characters
  - now reused by both `/Users/seanhan/Documents/Playground/src/knowledge/knowledge-service.mjs` and `/Users/seanhan/Documents/Playground/src/planner/knowledge-bridge.mjs`, but not wired into SQLite persistence or company-brain approval/governance paths
...
- `/Users/seanhan/Documents/Playground/src/knowledge/knowledge-service.mjs`
  ...
  - also exposes `queryKnowledgeWithSnippet(keyword)` for a bounded top-3 `{ id, snippet }` preview over the ranked results; this path now tries small block windows around the first matching block before falling back to a sentence window, then runs those candidates through `cleanSnippet(...)` and keeps the first non-low-value preview
...
- `/Users/seanhan/Documents/Playground/src/company-brain-memory-authority.mjs`
  ...
  - exposes `writeMemory({ key, value, source })`, `readMemory({ key })`, and `listMemoryByPrefix({ prefix })`
  - stores entries in `globalThis.__company_brain_memory__`
  - now also serves small local read-through cache helpers in `session-scope-store.mjs` and `executive-memory.mjs`
