...
- Responsibility:
  ...
  - keep `mutation-runtime.mjs` as a narrow passthrough wrapper only; today it is wired only around the `create_doc` HTTP execute path, returns `missing_execute` when no executor is supplied, and otherwise forwards into the existing route-local `executeLarkWrite(...)` flow while wrapping the downstream result with passthrough execution metadata; the HTTP route then unwraps that nested write envelope before returning the existing API response shape
