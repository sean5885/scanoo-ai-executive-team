...
- Responsibility:
  ...
  - the same planner contract/runtime line now also keeps `create_doc` governance aligned as checked-in metadata (`external_write=true`, `confirm_required=true`, `review_required=conditional`) across `planner_contract.json`, planner tool registry, and route contracts, while keeping the actual runtime gate in the existing document-create path unchanged
