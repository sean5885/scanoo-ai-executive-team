...
- Responsibility:
  ...
  - `src/read-runtime.mjs` now owns the first checked-in unified read-runtime entry for company-brain reads: it accepts canonical read requests, fixes `primary_authority = "mirror"` with `authorities_attempted = ["mirror"]`, delegates all supported reads to `src/company-brain-query.mjs` as the only mirror reader, and returns a canonical runtime envelope without live fallback or multi-authority mixing
