...
- Current truth:
  ...
  - `/Users/seanhan/Documents/Playground/src/lane-executor.mjs` now treats diagnose doc refs more aggressively than compare: when `plugin_context.document_refs` already exposes a direct token it uses that `document_id`, and when the handoff only preserves `title` it now fail-soft mirror-searches that title to recover one bounded `document_id`
  - once `scanoo-diagnose` has a resolved doc id and explicit user auth, the same wrapper now forces `/Users/seanhan/Documents/Playground/src/read-runtime.mjs -> readDocumentFromRuntime(...)` unless planner already executed a doc-read action; on that fallback path, explicit user auth is resolved from the plugin handoff envelope first and session-persisted explicit auth second, instead of relying on tenant/generic auth only; only when no id can be resolved does it fall back to the older planner-backed reply without official doc hydration
