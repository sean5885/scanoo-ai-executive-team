...
- Responsibility:
  ...
  - `src/read-runtime.mjs` now owns the checked-in unified read-runtime entry for single-authority reads: company-brain actions still fix `primary_authority = "mirror"` with `authorities_attempted = ["mirror"]` and delegate only to `src/company-brain-query.mjs`, while live doc/comment actions fix `primary_authority = "live"` with `authorities_attempted = ["live"]` and delegate only to `src/lark-content.mjs`; live reads require `freshness = "live_required"`, and the runtime does not mix mirror with live fallback in the same read
