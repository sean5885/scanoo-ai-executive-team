...
- Current truth:
  ...
  - the previously added planner-first DM health gate remains as local legacy state in `/Users/seanhan/Documents/Playground/src/lane-executor.mjs`, but generic DM no longer depends on planner-first at all for normal answer quality; direct-message text now prefers `executePersonalAssistant(...)` direct replies unless the turn is an explicit knowledge/tool/workflow case
  ...
  - `/Users/seanhan/Documents/Playground/src/lane-executor.mjs` now also emits DM-experience trace events (`personal_dm_path_decision`, `personal_dm_path_selected`, `personal_dm_planner_health_observed`, `model_first_dm_ingress_selected`) into the existing local runtime trace surface; these are request-trace evidence only, not a production telemetry pipeline
