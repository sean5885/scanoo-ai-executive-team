...
- `/Users/seanhan/Documents/Playground/src/knowledge/knowledge-service.mjs`
  ...
  - also exposes synchronous `queryKnowledgeWithContext(keyword)` as a filtered contextual preview helper that first tries the raw keyword, then fail-soft expands a small checked-in normalization set for matching Chinese terms (`交付`, `流程`, `設計`, `商機`, `管理`, `系統`, `穩不穩`, `運行情況`) plus any ASCII tokens already present in the query so mixed-language and runtime-health lookups can reuse alternate terms, filters each variant so snippets still contain that active query term and excludes `oauth` snippets unless the active query itself is `oauth`, deduplicates by `id`, and then removes very short fragments, table-row-like snippets, bare path/path-like labels, and empty metadata labels before returning the first bounded rows to planner-side callers
  - not connected to sync ingestion, SQLite persistence, planner routes, or company-brain approval/governance paths
