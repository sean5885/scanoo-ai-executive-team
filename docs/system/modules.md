...
- Responsibility:
  ...
  - that `agent_execution` field now stays bounded and deterministic: `src/planner/agent-executor.mjs` derives the checked-in lane-to-agent/action mapping, and `src/planner/agent-runtime.mjs` wraps it with local placeholder `result` payloads instead of invoking live agent handoff/runtime ownership
