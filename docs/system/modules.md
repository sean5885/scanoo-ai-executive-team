...
- Current truth:
  ...
  - `/Users/seanhan/Documents/Playground/src/lane-executor.mjs` now also emits DM-experience trace events (`personal_dm_path_decision`, `personal_dm_path_selected`, `personal_dm_planner_health_observed`) into the existing local runtime trace surface; these are request-trace evidence only, not a production telemetry pipeline
  ...
  - attachment/modality ingress now has a minimal PDF recognition path:
    ...
    - text-only PDF follow-up asks (for example `請做深讀`, `幫我解讀一下這個`) now attempt recent-message PDF context recovery in `/Users/seanhan/Documents/Playground/src/lane-executor.mjs`, so the lane can continue reading the latest file-card PDF without requiring users to re-send attachment cards
