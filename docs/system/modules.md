...
- Current truth:
  ...
  - attachment/modality ingress now has a minimal PDF recognition path:
    ...
    - text-only PDF follow-up asks (for example `請做深讀`) now attempt recent-message PDF context recovery in `/Users/seanhan/Documents/Playground/src/lane-executor.mjs`, so the lane can continue reading the latest file-card PDF without requiring users to re-send attachment cards
