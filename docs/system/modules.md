...
- Responsibility:
  ...
  - confirmed apply now records nested mutation audit entries for `update_document` / `resolve_comment`, and uses a compensating rollback helper to restore the document body plus reopen already-resolved comments when a later nested step fails
...
- Responsibility:
  ...
  - confirmed meeting write now records nested mutation audit entries and deletes a newly created meeting doc or restores the previous doc body if a later nested writeback step fails
