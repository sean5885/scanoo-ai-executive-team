...
- Current truth:
  ...
  - `scanoo-compare` still reuses that same answer-edge helper, but now has one extra fail-soft branch in `/Users/seanhan/Documents/Playground/src/lane-executor.mjs`: when the compare reply lacks usable evidence and did not already resolve to a doc-read action, it first hard-shapes the fallback search query by extracting up to two `*店` compare targets plus matched metric terms from `流量 / 轉化 / 留存 / 排名`, strips the minimal stopwords `比較 / 一下 / 幫我 / 看看`, prefers the form `A店 vs B店 + 指標`, and only then calls the checked-in mirror read helper `/Users/seanhan/Documents/Playground/src/read-runtime.mjs -> searchCompanyBrainDocsFromRuntime(...)`; if the pair cannot be recovered, it fail-soft falls back to the stopword-stripped query instead of the full user sentence
  ...
  - when that diagnose-lane reply still lands in an insufficient-evidence state and has not already resolved to a document-read action, the same wrapper now fail-soft re-enters the checked-in official read path via `/Users/seanhan/Documents/Playground/src/read-runtime.mjs -> readDocumentFromRuntime(...)`; it only does so when the current event, bounded `plugin_context.document_refs`, or referenced message resolves to a document id and the lane has explicit user auth, then returns one deterministic diagnose-shaped five-section reply that cites the official doc read without over-claiming a root cause
