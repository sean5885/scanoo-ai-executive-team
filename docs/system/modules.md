...
- Responsibility:
  ...
  - keep `mutation-runtime.mjs` as a narrow passthrough wrapper only; today it is wired only around the `create_doc` HTTP execute path and forwards straight into the existing route-local `executeLarkWrite(...)` flow
