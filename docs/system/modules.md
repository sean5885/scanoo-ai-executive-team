...
- Current truth:
  ...
  - `decision-engine-promotion` v1 now adds one gated override layer over advisor diagnostics:
    ...
    - the same boundary now also emits `promotion_audit` / `promotion_audit_summary` per step:
      ...
      - fixed rollback safety v1 (`N=3` consecutive ineffective audits per promoted action) disables future promotion for that action and falls back to advisory-only, without retroactively changing already-executed actions
