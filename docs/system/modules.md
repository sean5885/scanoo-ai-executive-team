...
- Responsibility:
  ...
  - keep `mutation-runtime.mjs` as a narrow execution-mode scaffold only; today it is wired only around the `create_doc` HTTP execute path, returns `missing_execute` when no executor is supplied, derives `meta.execution_mode` from `context.execution_mode` with a default of `passthrough`, records `meta.duration_ms` around the executor call, and now explicitly marks the controlled branch by passing `controlled: true` into the downstream executor while keeping the same route-local `executeLarkWrite(...)` destination; fail-soft still returns `execution_failed` if that executor throws, and the HTTP route then unwraps the nested write envelope before returning the existing API response shape
