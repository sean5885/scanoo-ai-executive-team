...
- Responsibility:
  ...
  - keep `mutation-runtime.mjs` as a narrow execution-mode scaffold only; today it is wired only around the `create_doc` HTTP execute path, returns `missing_execute` when no executor is supplied, derives `meta.execution_mode` from `context.execution_mode` with a default of `passthrough`, and both the current `passthrough` and placeholder `controlled` branches still forward into the existing route-local `executeLarkWrite(...)` flow; the HTTP route then unwraps that nested write envelope before returning the existing API response shape
