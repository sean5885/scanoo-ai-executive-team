...
- Current truth:
  ...
  - once `scanoo-diagnose` has a resolved doc id and explicit user auth, the same wrapper now forces `/Users/seanhan/Documents/Playground/src/read-runtime.mjs -> readDocumentFromRuntime(...)` unless planner already executed a doc-read action; on that fallback path, explicit user auth is resolved from the plugin handoff envelope first and session-persisted explicit auth second, instead of relying on tenant/generic auth only; if docs search still cannot resolve a document id, the lane now returns one bounded diagnose-contract missing-document reply instead of dropping to a generic fallback
