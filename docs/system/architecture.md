...
- `openclaw-plugin/lark-kb`
  ...
  - when that `scanoo-diagnose` reply still lands in an insufficient-evidence state and has not already executed a document-read action, the same wrapper now fail-soft re-enters the checked-in official live read helper `readDocumentFromRuntime(...)`; only when a doc id can be resolved from the current message, bounded `plugin_context.document_refs`, or referenced upstream message and explicit user auth is still available does it return a deterministic diagnose-shaped five-section reply that cites the official read as evidence while keeping the diagnosis non-conclusive
