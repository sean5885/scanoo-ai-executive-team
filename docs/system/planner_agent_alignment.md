...
Separately, `/Users/seanhan/Documents/Playground/src/planner/knowledge-bridge.mjs` now exists as a tiny planner-side local adapter over `/Users/seanhan/Documents/Playground/src/knowledge/knowledge-service.mjs`: `plannerAnswer({ keyword, question })` prefers an explicit `keyword`, otherwise fail-soft tries `/Users/seanhan/Documents/Playground/src/planner/intent-parser.mjs` to extract a document-search keyword from `question`, then expands that final keyword through `/Users/seanhan/Documents/Playground/src/planner/query-rewrite.mjs` before querying the same cached `docs/system` in-memory preview rows once per expanded key through `queryKnowledgeWithContext(...)`. The merged local rows are deduplicated by `id` before summarization. Inside that local helper, case-insensitive doc hits are reordered through `/Users/seanhan/Documents/Playground/src/knowledge/rank-results.mjs`, which boosts direct keyword matches, earlier keyword hits near the start of the document, repeated exact matches, and a small boundary-like bonus when the keyword appears adjacent to spaces. Snippet extraction then strips leading absolute `/Users/...` path lines and leading heading/metadata labels such as bare slash labels or `runtime / monitoring - Purpose:` prefixes, while the context helper first keeps only snippets that still contain the keyword and excludes `oauth` snippets unless the keyword itself is `oauth`, then drops very short fragments, table-row-like snippets, bare path/path-like labels, and empty metadata labels before returning rows. That parser no longer carries its own inline allowlist; it now reuses the shared `/Users/seanhan/Documents/Playground/src/config/tech-terms.mjs` registry through `/Users/seanhan/Documents/Playground/src/utils/pick-tech-term.mjs`, with longest-term-first matching so `entry os` or `scanoo entry os` are not shadowed by the broader `entry` substring. `/Users/seanhan/Documents/Playground/src/planner/query-rewrite.mjs` now also exists as a tiny deterministic local helper that expands a small checked-in alias map for `routing`, `planner`, `sop`, and `okr`, or derives one mapped expansion set from `pickTechTerm(question)` before falling back to `[keyword]` or `[]`. The planner-side local summary path still summarizes rows through `/Users/seanhan/Documents/Playground/src/planner/llm-summary.mjs`, which now uses the shared `/Users/seanhan/Documents/Playground/src/llm/generate-text.mjs` helper for the repo-wide text-model path, and falls back to `/Users/seanhan/Documents/Playground/src/planner/answer-builder.mjs` when generation fails. `/Users/seanhan/Documents/Playground/src/planner/answer-builder.mjs` exposes `buildAnswer(keyword, results) -> string`, returning either a fixed Chinese no-result line or a numbered summary over the same `{ id, snippet }` preview rows with a count-based intro and snippet cleanup that strips inline code spans, local absolute-path fragments, excess whitespace, leading punctuation noise, trailing separator noise, stray spaces before punctuation, repeated commas, and dangling trailing conjunction/placeholder tails. These helpers are not part of the current public planner runtime surface, are not wired into `executive-planner.mjs`, and do not change `planner_contract.json`, planner flow routing, or company-brain governance boundaries.
