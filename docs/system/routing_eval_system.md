...
目前 eval set 共有 102 筆，覆蓋：

- dataset validation 仍維持 bounded checked-in pack，當前允許範圍是 `50~110` 筆
...
- delivery / onboarding 知識查詢的自然中文問句，像 `請整理交付驗收流程`、`把 onboarding 流程講給我聽`、`導入 SOP 在哪`，即使沒有明寫 `文件` 也會進 `knowledge_assistant`
...
- `release-check` drilldown 的 representative case 若集中在同一個 `doc` / `meeting` / `runtime` 類別，先補那一類 fixture
- 只有在 latest snapshot 的 miss case 反映 checked-in deterministic 行為其實是合理且穩定、但 dataset 尚未覆蓋時，才補 fixture
- 若 miss 代表 rule precedence、lane、planner action、error code 本身錯了，不補 fixture，改查 rule
...
- 若 `release-check` representative case 顯示同一 case id 在 latest snapshot 仍落到錯 lane / 錯 action / 錯 agent_or_tool，先查 `src/router.js`、`src/planner-*-flow.mjs`
- 若 case 其實是 planner contract / selector drift，而不是 routing eval dataset 問題，改走 planner diagnostics / contract 檢查，不在這裡補 fixture
