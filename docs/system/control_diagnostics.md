  ...
  - summary lists:
    ...
    - `basis_summary`
      - `warn_to_enforce_readiness`
        - fixed `real_request_backed_sample_progress` (例如 `0/20`)
      - `operational_debt`
        - 當 `meeting_confirm_write` real request-backed 樣本不足時，固定標記 `insufficient_real_request_backed_samples`
...
- `self-check` / `release-check` 的 human-readable summary 也會帶出：
  ...
  - `是否可作為 rollout 依據`
  - `warn->enforce readiness`（固定顯示 `meeting_confirm_write=<current>/<min>`）
  - `operational debt`（樣本不足時顯示 debt item）
  - `upgrade_ready_routes`
  - `high_risk_routes`
