...
- `/Users/seanhan/Documents/Playground/src/llm/generate-text.mjs` now provides a small shared text-generation helper for local modules: `generateText({ systemPrompt, prompt, sessionIdSuffix, temperature, topP, signal })` uses the repo-wide text-model settings (`MINIMAX_TEXT_MODEL` -> `DEEPSEEK_TEXT_MODEL` -> legacy `LLM_MODEL` fallback), resolves direct-provider auth from `LLM_API_KEY` first and compatible aliases (`DEEPSEEK_API_KEY`, `OPENAI_API_KEY`, `MINIMAX_API_KEY`, `KIMI_API_KEY`) second, and otherwise re-enters the checked-in OpenClaw text path, so planner-side helpers can reuse one stable text-generation contract instead of duplicating provider wiring
- the current checked-in local default points that shared text-model path at DeepSeek (`LLM_BASE_URL=https://api.deepseek.com`, `MINIMAX_TEXT_MODEL=deepseek-v4-pro`), while keeping the existing env key name for backward compatibility with the rest of the repo
- that OpenClaw text/classifier path now sanitizes the runtime config through `/Users/seanhan/Documents/Playground/src/openclaw-runtime-config.mjs` before invoking the CLI: the generated config strips Feishu/channel-only plugin coupling, filters stale `feishu_*` tool allowlist entries, and forces the agent default model back onto the currently configured provider-qualified primary model (for example `deepseek/deepseek-v4-pro`) so local text generation and semantic classification do not inherit unrelated broken channel config or expired Kimi defaults from the user-global OpenClaw profile
