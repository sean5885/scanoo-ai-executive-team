...
- `/Users/seanhan/Documents/Playground/src/knowledge/knowledge-service.mjs` now provides a tiny cached query wrapper: `getIndex()` lazily loads `./docs/system` once per process, `queryKnowledge(keyword)` runs case-insensitive keyword search over that cached in-memory index and then reorders the hits through `rankResults(hits, keyword)`, `queryKnowledgeWithSnippet(keyword)` returns a bounded top-3 `{ id, snippet }` preview that expands around the matched keyword, snaps outward to nearby line/sentence-style breaks when available, strips leading absolute `/Users/...` path lines, and removes leading heading/metadata labels such as bare slash labels or `runtime / monitoring - Purpose:` prefixes before returning snippet text; `queryKnowledgeWithContext(keyword)` remains a synchronous helper, first tries the raw keyword, then fail-soft expands a small checked-in normalization set over matching Chinese terms (`交付`, `流程`, `設計`, `商機`, `管理`, `系統`, `穩不穩`, `運行情況`) plus any ASCII tokens already present in the query so mixed-language lookups such as `OKR 設計` or `BD 商機管理` and runtime-health wording such as `系統現在穩不穩` can reuse alternate forms, filters each variant so snippets still contain that active query term and `oauth` snippets are dropped unless the active term itself is `oauth`, deduplicates by `id`, and keeps the first bounded top-3 rows after the existing low-value-snippet filter removes very short fragments, table-row-like snippets, bare path/path-like labels, and empty metadata labels
