...
- `/Users/seanhan/Documents/Playground/src/knowledge/knowledge-service.mjs` now provides a tiny cached query wrapper: `getIndex()` lazily loads `./docs/system` once per process, `queryKnowledge(keyword)` runs case-insensitive keyword search over that cached in-memory index, `queryKnowledgeWithSnippet(keyword)` returns a bounded top-3 `{ id, snippet }` preview that expands around the matched keyword and then snaps outward to nearby line/sentence-style breaks when available, and `queryKnowledgeWithContext(keyword)` applies a small low-value-snippet filter over that preview so planner-side callers drop very short fragments, table-row-like snippets, bare path/path-like labels, and empty metadata labels before returning contextual rows
