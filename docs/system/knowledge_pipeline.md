...
- `/Users/seanhan/Documents/Playground/src/planner/knowledge-bridge.mjs` now provides a tiny planner-side adapter over that same local helper: `plannerQueryKnowledge({ keyword })` returns `{ count, results }` using the cached in-memory `docs/system` index
