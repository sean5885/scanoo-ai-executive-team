...
- `/Users/seanhan/Documents/Playground/src/knowledge/snippet-cleaner.mjs` now provides a line-aware local snippet-cleanup helper: `cleanSnippet(text, keyword)` unwraps Markdown links/images while keeping visible text, strips heading markers, list shells, stray backticks, absolute local paths, navigation lines such as `Back to README.md`, checkpoint-style metadata lines, and module/title prefixes such as `runtime / monitoring - Purpose:`; it then drops dangling or path-only lines, merges the remaining meaningful lines, prefers the sentence segment containing the active keyword when available, trims broken leading/trailing fragments, and clamps the final preview near a sentence/line boundary at roughly 220 characters. This helper is now reused by both `/Users/seanhan/Documents/Playground/src/knowledge/knowledge-service.mjs` and `/Users/seanhan/Documents/Playground/src/planner/knowledge-bridge.mjs`, but is still not wired into SQLite persistence or company-brain routes
- `/Users/seanhan/Documents/Playground/src/knowledge/knowledge-service.mjs` now provides a tiny cached query wrapper: `getIndex()` lazily loads `./docs/system` once per process, `queryKnowledge(keyword)` runs case-insensitive keyword search over that cached in-memory index and then reorders the hits through `rankResults(hits, keyword)`, `queryKnowledgeWithSnippet(keyword)` now builds a few small block windows around the first matching block before falling back to a sentence window, runs each candidate through `cleanSnippet(...)`, and returns the first non-low-value preview as a bounded `{ id, snippet }` result; `queryKnowledgeWithContext(keyword)` remains a synchronous helper, first prioritizes a small checked-in alias set for common ASCII topic terms (`okr`, `bd`, `sop`, `delivery`, `onboarding`), then fail-soft expands a small checked-in normalization set over matching Chinese terms (`交付`, `流程`, `設計`, `商機`, `管理`, `系統`, `穩不穩`, `運行情況`), but now suppresses generated generic `business` variants when the query is already in explicit BD / 商機 context unless the user typed `business` directly; only after that does it fall back to any ASCII tokens already present in the query plus the raw full query so mixed-language lookups such as `OKR 設計` or `BD 商機管理`, delivery-style wording such as `Scanoo 交付流程`, and runtime-health wording such as `系統現在穩不穩` can reuse alternate forms without letting broad brand-like terms dominate the first bounded rows; it still filters each variant so snippets contain that active query term, drops `oauth` snippets unless the active term itself is `oauth`, deduplicates by `id`, and round-robins across those bounded per-query buckets before the low-value-snippet filter removes very short fragments, table-row-like snippets, bare path/path-like labels, heading-only/runbook-like titles, and dangling metadata fragments
