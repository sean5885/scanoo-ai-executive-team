...
| `/api/monitoring/autonomy/final` | `GET` | read-only autonomy final pickup lookup by `trace_id`/`request_id` token (`X-Trace-Id`/`X-Request-Id` compatible) with bounded final-result projection | implemented |
| `/api/monitoring/learning` | `GET` | review-first learning summary from request traces | implemented |
| `/agent/improvements/learning/generate` | `POST` | persist learning-loop improvement proposals with risk-based mode (`tool_weight_adjustment` may auto-apply; high-risk routes stay review-gated) and additive replay effect evidence | implemented |
...
- when a timeout fallback hits a multi-intent request with only completable intents (for example summarize + draft), the answer boundary now still returns a partial draft deliverable plus one explicit summary-input next step instead of a full generic fail
