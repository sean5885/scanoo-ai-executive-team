...
| `/agent/lark-plugin/dispatch` | `POST` | official Lark plugin hybrid dispatch entry; normalizes plugin request/session metadata plus `requested_capability / capability_source`, uses explicit or plugin-inferred capability to decide `knowledge_answer` vs `lane_backend` vs `plugin_native`, records observability, then either executes the bounded backend path or returns a plugin-native forward decision | implemented |
