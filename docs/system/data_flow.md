...
13. `renderPlannerUserFacingReplyText(...)` keeps fixed public order:
   ...
   - final source rendering preserves both canonical source objects and already-normalized source strings from upstream answer shaping; it no longer drops evidence lines merely because they were normalized earlier in the pipeline
...
4. after meeting / workflow fast-path checks, the DM entrypoint now gives one model-first ingress pass in `/Users/seanhan/Documents/Playground/src/lane-executor.mjs -> planModelFirstDMIngress(...)` before same-session shortcutting or deterministic attachment heuristics
5. that ingress only activates when:
   - scope is direct-message
   - the primary text model is available
   - the currently expected owner is still `personal-assistant` or `knowledge-assistant`
   - no explicit workflow mode / active workflow task is already steering the turn
6. the ingress model emits one bounded route decision:
   - `pdf_task`
   - `image_task`
   - `knowledge_assistant`
   - `personal_assistant`
...
10D. on that same personal-assistant model-first path, generic external company / market / competitor / strategy asks now prefer synthesis-first prompting: when the user did not explicitly ask for verified latest numbers, the model is instructed to provide a conditional analytical summary first and keep freshness / evidence limits in `重點` or `下一步` instead of opening with a pure `cannot browse` capability disclaimer
...
11A. greeting/closing-only DM turns are still excluded from the bounded skill-intent classifier pre-pass, but once they stay on the personal-assistant lane they can still be answered by the primary text model instead of being forced into the older canned greeting/ack template
...
- when personal DM auth falls back to tenant/app scope because user OAuth refresh is unavailable, the lane now only hard-blocks reads that truly require user-private data (for example recent dialogue / calendar / task reads); generic text analysis, critique, rewriting, and strategy-style turns still continue into the primary text-model path instead of being swallowed by the old tenant-mode scaffold
