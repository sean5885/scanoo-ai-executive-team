...
const tempDir = mkdtempSync(path.join(tmpdir(), "playground-governance-eval-"));
...
async function runKnowledgeRounds() {
  const workflowKey = "knowledge-lane:evaluate-long-task";
  const rounds = [];
  for (let round = 1; round <= 10; round += 1) {
    ...
    rounds.push({
      round,
      stage: prompt.governance.stage,
      raw_tokens: prompt.governance.rawTokens,
      final_tokens: prompt.governance.finalTokens,
    });
  }
  return {
    checkpoint: await getWorkflowCheckpoint(workflowKey),
    rounds,
  };
}
...
try {
  const knowledge = await runKnowledgeRounds();
  const rewrite = await runRewriteRounds();
  const result = {
    knowledge: {
      rounds: knowledge.rounds,
      checkpoint_completed_count: knowledge.checkpoint?.completed?.length || 0,
      checkpoint_pending_count: knowledge.checkpoint?.pending?.length || 0,
      max_final_tokens: Math.max(...knowledge.rounds.map((item) => item.final_tokens)),
      growth_from_first_round:
        knowledge.rounds.at(-1).final_tokens - knowledge.rounds[0].final_tokens,
    },
    rewrite: {
      rounds: rewrite.rounds,
      checkpoint_completed_count: rewrite.checkpoint?.completed?.length || 0,
      checkpoint_pending_count: rewrite.checkpoint?.pending?.length || 0,
      max_final_tokens: Math.max(...rewrite.rounds.map((item) => item.final_tokens)),
      growth_from_first_round:
        rewrite.rounds.at(-1).final_tokens - rewrite.rounds[0].final_tokens,
    },
  };
